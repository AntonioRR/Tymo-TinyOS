...
implementation {
  ...
  enum {
    WAIT_BEFORE_RETRY = LPL_DEF_LOCAL_WAKEUP * 2,  // without low power 100, with low power 1024.
 ...
 enum {
  ...
  //  MAX_WAIT = 10 * WAIT_BEFORE_RETRY,
	...
	event void SubSend.sendDone(message_t * msg, error_t e){
		...
		if ((e == SUCCESS) && acks) {          //message sent.
			if( !(call Acks.wasAcked(msg)) ){   // Msg was not acknowledged.
				...
				call DelayTimer.startOneShot(TIMER_RETX_MSG);
				return;	
			} 
	...
	event void DelayTimer.fired(){
		...
		typebuf = call AMPacket.type(currentReSendMsg); //adding by Antonio Rosa 20121005
		...
		switch(call RouteSelect.selectRoute(waiting, addrWaiting, &amWaiting)){
			case FW_SEND:
				...
				// currentSendMsg = waiting; //adding by Antonio Rosa
					acks = DYMO_LINK_FEEDBACK && (call Acks.requestAck(waiting) == SUCCESS); //adding by Antonio Rosa
					typebuf = amWaiting;		      					//adding by Antonio Rosa
					call SubSend.send(call AMPacket.destination(waiting), waiting, lenWaiting);
				} 

				else if (signal Intercept.forward[amWaiting](waiting, call PPacket.getPayload(waiting, call PPacket.payloadLength(waiting)), call PPacket.payloadLength(waiting))) {
					//currentSendMsg = waiting; //adding by Antonio Rosa	
					acks = DYMO_LINK_FEEDBACK && (call Acks.requestAck(waiting) == SUCCESS); //adding by Antonio Rosa
					typebuf = amWaiting;		      					//adding by Antonio Rosa
					call SubSend.send(call AMPacket.destination(waiting), waiting, lenWaiting);
