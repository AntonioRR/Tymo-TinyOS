...
module DymoEngineM {
  ...
  uses {
    ...
    interface AMPacket;
    interface Receive;
    interface InternalFlash; // Modified by Antonio Rosa. Now we save/read num_seq to/from internal EEPROM
    interface PacketAcknowledgements as Ack;  //Now we send RREP and RERR with ack.
   // interface Mount;    Modified by Antonio Rosa. Now we save/read num_seq to/from internal EEPROM
   // interface ConfigStorage;  Modified by Antonio Rosa. Now we save/read num_seq to/from internal EEPROM
    interface Leds;
...
implementation {
  ...
  uint32_t rreq_time;
	...
	enum{
		DYMO_NUM_SEQ_ADDRESS = 0x130};  //Changed by Antonio Rosa.
  ...
  command error_t SplitControl.start(){
   ...
   // return call Mount.mount();
    call InternalFlash.read((uint16_t *)DYMO_NUM_SEQ_ADDRESS, &me.seqnum, sizeof(me.seqnum));
    if (me.seqnum == 0xFFFF)
        	me.seqnum = 1;
  ...
  //  call ConfigStorage.write(0x0, &me.seqnum, sizeof(me.seqnum));
       call InternalFlash.write((uint16_t *)DYMO_NUM_SEQ_ADDRESS,&me.seqnum,sizeof(me.seqnum));
  }
...
/*  event void ConfigStorage.writeDone(storage_addr_t addr, void *buf, 
  ...
  /* Send a RREP to buf_info */ //Now we add ack  by Antonio Rosa
  task void issueRREP(){
    atomic {
      if(busySend)
	post issueRREP();
	...
	if(buf_target.address)
	 ...
	 call Ack.requestAck(&buf_packet);
...
#if DYMO_APPEND_INFO
      call DymoPacket.addInfo(&fw_msg, &me);  //modificado por Antonio para poder compilar
#endif
  ...
  event void AMSend.sendDone(message_t *msg, error_t error){
   if (call DymoPacket.getType(msg) == DYMO_RREP ){
      if( !(call Ack.wasAcked(msg)) ){
		count ++;
			if (count > 4/*MAX_RETX*/){
			  ...
			  call RespTimer.startOneShot(/*TIMER_RETX_MSG*/20);
      }
	else {
	               /*  atomic {
			      busySend = FALSE;
